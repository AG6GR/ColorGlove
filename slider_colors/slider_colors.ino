...
//change color of LED using 3 sliders
//getRGB from http://www.kasperkamperman.com/blog/arduino/arduino-programming-hsb-to-rgb/ 
...
const int ledPinG   = 4; // pwm pin with green led
const int ledPinB   = 5; // pwm pin with blue led
const int buttonPin = 23; // pin hooked to the button
...
void loop()  { 
  ...
  sensorValH = analogRead(0);
  sensorValS = analogRead(1);
  sensorValB = analogRead(2);
  ...
  hue        = map(sensorValH, 15, 980, 0, 359); // hue is a number between 0 and 360
  //Serial.println(String(sensorValH));
  saturation = map(sensorValS, 0, 1023, 0, 254);   // saturation is a number between 0 - 255
  brightness = map(sensorValB, 0, 1023, 0, 254);   // value is a number between 0 - 255
 
  getRGB(hue,saturation,brightness,rgb_colors);   // converts HSB to RGB

 if (isActive)
 {
  analogWrite(ledPinR, 255 - rgb_colors[0]);            // red value in index 0 of rgb_colors array
  analogWrite(ledPinG, 255 - rgb_colors[1]);            // green value in index 1 of rgb_colors array
  analogWrite(ledPinB, 255 - rgb_colors[2]);            // blue value in index 2 of rgb_colors array
 }
 
  //Serial.println("RGB: " + String(rgb_colors[0]) + ", " + String(rgb_colors[1]) + ", " + String(rgb_colors[2]));
  Serial.println("HSB: " + String(hue) + ", " + String(saturation) + ", " + String(brightness));
  delay(10);
}
